  ...
  if (!gamePause) {
    if (orderList_timeLeft_ms[orderListID - 1] > 0) {
      ...
      let progressBar = document.getElementById(
        `orderList_${orderListID}_progressBar`
      );
      let value =
        (orderList_timeLeft_ms[orderListID - 1] /
          1000 /
          orderList_time_s[orderListID - 1]) *
        100;
  ...
  for (let i = 0; i < orderList.length && finish == false; ++i) {
    for (let j = 0; j < 2 && finish == false; ++j) {
      if (orderListStatus[i][j] == false && orderList[i][j] == burgeID) {
        ...
        document
          .getElementById(`plate_${plateID}`)
          .setAttribute("src", "img/plate.png");
        setTimeout(function () {
...
function checkOrderFinish(orderListID) {
  if (
    orderListStatus[orderListID - 1][0] == true &&
    orderListStatus[orderListID - 1][1] == true
  ) {
...
function updateOrderList() {
  ...
  for (i; i < orderList.length; ++i) {
    // timer
    document.getElementById(`orderList_${i + 1}_progress`).style.visibility =
      "visible";
    // setTimeout(function(){
    ...
    // item_1
    document
      .getElementById(`orderItem_${i + 1}-1`)
      .setAttribute("src", `./img/${burgerStatusToName[orderList[i][0]]}.png`);
    ...
    // item_2
    document
      .getElementById(`orderItem_${i + 1}-2`)
      .setAttribute("src", `./img/${burgerStatusToName[orderList[i][1]]}.png`);
    ...
    // orderList
    document.getElementById("orderList_" + (i + 1)).style.visibility =
      "visible";
  }
  ...
  switch (ingredient) {
    case "hambergerBun":
      for (let i = 0; i < maxBurgerNumber; ++i) {
        ...
        if (burgerStatus[i] == 0) {
          document
            .getElementById("plate_" + (i + 1))
            .setAttribute("src", "./img/burger.png");
          burgerStatus[i] = 1;
          break;
        }
      }
    ...
    case "rawMeat":
      for (let i = 0; i < maxMeatNumber; ++i) {
        ...
        if (meatStatus[i] == 0) {
          document
            .getElementById("meat_" + (i + 1))
            .setAttribute("src", "./img/meat_nc.png");
          // 更改 progressBar 顏色
          document
            .getElementById(`meat_${i + 1}_progressBar`)
            .classList.remove("progress-bar-danger");
          document
            .getElementById(`meat_${i + 1}_progressBar`)
            .classList.add("progress-bar-success");
          // 顯示進度條
          document.getElementById(`meat_${i + 1}_progress`).style.visibility =
            "visible";
          meatStatus[i] = 1;
          cookMeat(i + 1);
          break;
      ...
      if (meatStatus[0] == 2) {
        for (let i = 0; i < maxBurgerNumber; ++i) {
          if (burgerStatus[i] == 1) {
            ...
            // 隱藏進度條
            document.getElementById("meat_1_progress").style.visibility =
              "hidden";
            // 歸零進度條
            ...
            document
              .getElementById("plate_" + (i + 1))
              .setAttribute("src", "./img/burger_m.png");
            burgerStatus[i] = 2;
            meatStatus[0] = 0;
      ...
      if (meatStatus[1] == 2) {
        for (let i = 0; i < maxBurgerNumber; ++i) {
          if (burgerStatus[i] == 1) {
            ...
            // 隱藏進度條
            document.getElementById("meat_2_progress").style.visibility =
              "hidden";
            // 歸零進度條
      ...
      if (meatStatus[2] == 2) {
        for (let i = 0; i < maxBurgerNumber; ++i) {
          if (burgerStatus[i] == 1) {
            ...
            // 隱藏進度條
            document.getElementById("meat_3_progress").style.visibility =
              "hidden";
            // 歸零進度條
      ...
      if (meatStatus[3] == 2) {
        for (let i = 0; i < maxBurgerNumber; ++i) {
          if (burgerStatus[i] == 1) {
            ...
            // 隱藏進度條
            document.getElementById("meat_4_progress").style.visibility =
              "hidden";
            // 歸零進度條
    ...
    case "cheese":
      for (let i = 0; i < maxBurgerNumber; ++i) {
        ...
        if (burgerStatus[i] == 2) {
          document
            .getElementById("plate_" + (i + 1))
            .setAttribute("src", "./img/burger_m_c.png");
        ...
        if (burgerStatus[i] == 4) {
          document
            .getElementById("plate_" + (i + 1))
            .setAttribute("src", "./img/burger_m_c_o.png");
    ...
    case "onion":
      for (let i = 0; i < maxBurgerNumber; ++i) {
        ...
        if (burgerStatus[i] == 2) {
          document
            .getElementById("plate_" + (i + 1))
            .setAttribute("src", "./img/burger_m_o.png");
    ...
    case "tomato":
      for (let i = 0; i < maxBurgerNumber; ++i) {
        ...
        if (burgerStatus[i] == 2) {
          document
            .getElementById("plate_" + (i + 1))
            .setAttribute("src", "./img/burger_m_t.png");
        ...
        if (burgerStatus[i] == 7) {
          document
            .getElementById("plate_" + (i + 1))
            .setAttribute("src", "./img/burger_m_l_t.png");
    ...
    case "lettuce":
      for (let i = 0; i < maxBurgerNumber; ++i) {
        ...
        if (burgerStatus[i] == 2) {
          document
            .getElementById("plate_" + (i + 1))
            .setAttribute("src", "./img/burger_m_l.png");
...
function cookMeatAnimation(meatID) {
  if (!gamePause) {
    if (meat_timeLeft_ms[meatID - 1] > 0) {
      ...
      let value =
        ((cookingTime - meat_timeLeft_ms[meatID - 1] / 1000) / cookingTime) *
        100;
      progressBar.style.width = `${value}%`;
    } else {
...
function meatIsCooked(meatID) {
  document
    .getElementById("meat_" + meatID)
    .setAttribute("src", "./img/meat_c.png");
...
function resetProgressBar(meatID) {
  if (!gamePause) {
    if (meat_timeLeft_ms[meatID - 1] > 0) {
      ...
      clearInterval(meat_timers[meatID - 1]);
      // 隱藏進度條
      document.getElementById(`meat_${meatID}_progress`).style.visibility =
        "hidden";
      // 歸零
      document.getElementById(`meat_${meatID}_progressBar`).style.width = "0%";
      // 更改 progressBar 顏色
      document
        .getElementById(`meat_${meatID}_progressBar`)
        .classList.remove("progress-bar-success");
      document
        .getElementById(`meat_${meatID}_progressBar`)
...
function meatOvercookedAnimation(meatID) {
  if (!gamePause) {
    if (meat_timeLeft_ms[meatID - 1] > 0) {
      ...
      let progressBar = document.getElementById(`meat_${meatID}_progressBar`);
      let value =
        ((overcookedTime - meat_timeLeft_ms[meatID - 1] / 1000) /
          overcookedTime) *
        100;
      progressBar.style.width = `${value}%`;
    } else {
...
function meatIsOvercooked(meatID) {
  ...
  document
    .getElementById("meat_" + meatID)
    .setAttribute("src", "./img/meat_oc.png");
}
...
function enableAllButton() {
  document
    ...
    .setAttribute("onclick", "addIngredient('cheese')");
  document
    .getElementById("onion")
    .setAttribute("onclick", "addIngredient('onion')");
  document
    .getElementById("tomato")
    .setAttribute("onclick", "addIngredient('tomato')");
  document
    .getElementById("lettuce")
    .setAttribute("onclick", "addIngredient('lettuce')");
  document.getElementById("plate_1").setAttribute("onclick", "finishBurger(1)");
  document
    .getElementById("plate_1")
    .setAttribute("ondbclick", "moveBurgerToTrash(1)");
  document.getElementById("plate_2").setAttribute("onclick", "finishBurger(2)");
  document
    .getElementById("plate_2")
    .setAttribute("ondbclick", "moveBurgerToTrash(2)");
  document
    .getElementById("hamburgerBun")
    .setAttribute("onclick", "addIngredient('hambergerBun')");
  document
    .getElementById("rawMeat")
    .setAttribute("onclick", "addIngredient('rawMeat')");
  document
    .getElementById("meat_1")
    .setAttribute("onclick", "addIngredient('meat_1')");
  document
    .getElementById("meat_1")
    .setAttribute("ondbclick", "moveMeatToTrash(1)");
  document
    .getElementById("meat_2")
    .setAttribute("onclick", "addIngredient('meat_2')");
  document
    .getElementById("meat_2")
    .setAttribute("ondbclick", "moveMeatToTrash(2)");
  document
    .getElementById("meat_3")
    .setAttribute("onclick", "addIngredient('meat_3')");
  document
    .getElementById("meat_3")
    .setAttribute("ondbclick", "moveMeatToTrash(3)");
  document
    .getElementById("meat_4")
    .setAttribute("onclick", "addIngredient('meat_4')");
  document
    .getElementById("meat_4")
    .setAttribute("ondbclick", "moveMeatToTrash(4)");
}
...
function starAnimation() {
  if (starCount <= starNumber) {
    if (starCount > 0) {
      document
        .getElementById(`star${starCount}`)
        .setAttribute("src", "img/star1.png");
    }
    ++starCount;
  } else {
